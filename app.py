...
with st.form("sales_form"):
    ...
    with col3:
        ...
        # 🔸 หมายเหตุแบบตัวเลือก + ช่องรายละเอียด
        if edit_mode and isinstance(edit_row["หมายเหตุ"], str) and ":" in edit_row["หมายเหตุ"]:
            default_note_type, default_note_detail = edit_row["หมายเหตุ"].split(":", 1)
            default_note_type = default_note_type.strip()
            default_note_detail = default_note_detail.strip()
        else:
            default_note_type, default_note_detail = "", ""

        หมายเหตุ_ตัวเลือก = st.selectbox(
            "หมายเหตุ",
            ["", "คืนสินค้า", "พัสดุตีกลับ", "ยกเลิกออเดอร์", "อื่นๆ"],
            index=(
                ["", "คืนสินค้า", "พัสดุตีกลับ", "ยกเลิกออเดอร์", "อื่นๆ"].index(default_note_type)
                if default_note_type in ["คืนสินค้า", "พัสดุตีกลับ", "ยกเลิกออเดอร์", "อื่นๆ"]
                else 0
            )
        )
        ...
        if หมายเหตุ_ตัวเลือก:
            หมายเหตุ_รายละเอียด = st.text_input("รายละเอียดเพิ่มเติม", value=default_note_detail)

    submitted = st.form_submit_button("💾 บันทึกข้อมูล")

if submitted:
    หมายเหตุรวม = f"{หมายเหตุ_ตัวเลือก}: {หมายเหตุ_รายละเอียด}" if หมายเหตุ_ตัวเลือก else ""
    new_data = pd.DataFrame([[วันที่สั่งซื้อ, ลำดับ, เลขที่ใบกำกับ, Seller,
        เลขที่คำสั่งซื้อชื่อลูกค้า, รหัส, สี, Size, ราคาขาย, ส่วนลด,
        สุทธิ, วดป, จำนวนเงิน, ค่าขนส่งกทม, ค่าขนส่งตจว, เลขที่ใบโอน, หมายเหตุรวม]],
        columns=COLUMNS)
