...
if os.path.exists(DATA_FILE) and os.path.getsize(DATA_FILE) > 0:
    ...
    # ถ้าไม่มีคอลัมน์ "หมายเหตุ" ให้เพิ่มเข้าไป
...
with st.form("sales_form"):
    ...
    with col1:
        วันที่สั่งซื้อ = st.date_input("วันที่สั่งซื้อ", value=None if not edit_mode else pd.to_datetime(edit_row["วันที่สั่งซื้อ"]))
        ...
        Seller = st.selectbox("Seller", ["shopee", "lazada", "cent", "อื่นๆ"], 
                              index=0 if not edit_mode else ["shopee", "lazada", "cent", "อื่นๆ"].index(edit_row["Seller"]) if edit_row["Seller"] in ["shopee", "lazada", "cent", "อื่นๆ"] else 0)
    ...
    with col3:
        วดป = st.date_input("ว.ด.ป.", value=None if not edit_mode else pd.to_datetime(edit_row["ว.ด.ป."]))
    ...
    # หมายเหตุ (Dropdown + กล่องข้อความ)
    หมายเหตุ_ตัวเลือก = st.selectbox(
        "หมายเหตุ", ["", "คืนสินค้า", "พัสดุตีกลับ", "ยกเลิกออเดอร์", "อื่นๆ"],
        index=0 if not edit_mode else
        (["", "คืนสินค้า", "พัสดุตีกลับ", "ยกเลิกออเดอร์", "อื่นๆ"].index(str(edit_row["หมายเหตุ"]).split(":", 1)[0])
         if str(edit_row["หมายเหตุ"]).split(":", 1)[0] in ["คืนสินค้า", "พัสดุตีกลับ", "ยกเลิกออเดอร์", "อื่นๆ"]
         else 0)
    )

    หมายเหตุ_รายละเอียด = ""
    if หมายเหตุ_ตัวเลือก != "":
        หมายเหตุ_รายละเอียด = st.text_input("รายละเอียดเพิ่มเติม", 
                                           value="" if not edit_mode else (
                                               str(edit_row["หมายเหตุ"]).split(":", 1)[1].strip() 
                                               if ":" in str(edit_row["หมายเหตุ"]) else ""))
